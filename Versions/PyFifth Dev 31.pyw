    ...
    class Spellcasting:
        ...
        def CalculateSpellPoints(self):
            ...
            if self.SpellUsingSpellPointsBoxVar.get():
                # Validate Spell Slot Values
                if self.ValidSpellSlotsValues():
                    pass
                else:
                    return
        ...
        def ValidSpellSlotsValues(self):
            for Level in self.SpellSlotsList:
                try:
                    Slots = GlobalInst.GetStringVarAsNumber(Level.SlotsEntryVar)
                except:
                    messagebox.showerror("Invalid Entry", "Spell slots must be whole numbers when using spell points.")
                    return False
                if Slots < 0:
                    messagebox.showerror("Invalid Entry", "Spell slots must be greater than 0 when using spell points.")
                    return False
            return True
