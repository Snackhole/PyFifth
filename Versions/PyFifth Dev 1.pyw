...
class CharacterSheet:
    def __init__(self, master, Row, Column):
        ...
        self.CharacterSheetFrame.grid(row=self.Row, column=self.Column, sticky=NSEW)
    ...
    class CombatAndFeatures:
        def __init__(self, master):
            ...
            self.ACEntry = Entry(self.ACFrame, width=9, justify=CENTER, textvariable=self.ACEntryVar)
            ...
            self.InitiativeEntry = Entry(self.InitiativeFrame, width=9, justify=CENTER, textvariable=self.InitiativeEntryVar, cursor="dotbox")
            ...
            SavingAndOpeningInst.SavedData["OffenseNotes.Text"] = self.OffenseNotes
            SavingAndOpeningInst.SavedData["DefenseNotes.Text"] = self.DefenseNotes
            SavingAndOpeningInst.SavedData["FeaturesNotes"] = self.FeaturesNotes
    ...
    class Inventory:
        ...
        class InventoryEntry:
            ...
            def ConfigureMagicItem(self, event):
                # Create Window and Wait
                MagicItemMenuInst = self.MagicItemMenu(WindowInst, self.NameEntryVar, self.CategoryEntryVar, self.RarityEntryVar, self.DescriptionVar)
...
class StatModifier:
    ...
    def GetModifier(self):
        StrengthMod = self.GetSingleStatMod(Inst["AbilitiesAndSavingThrows"].StrengthEntry.AbilityEntryModifierVar, self.Variables["StrengthMultiplierEntryVar"], self.Variables["StrengthMultiplierRoundUpBoxVar"],
                                            self.Variables["StrengthMaxEntryVar"], self.Variables["StrengthMinEntryVar"])
        DexterityMod = self.GetSingleStatMod(Inst["AbilitiesAndSavingThrows"].DexterityEntry.AbilityEntryModifierVar, self.Variables["DexterityMultiplierEntryVar"], self.Variables["DexterityMultiplierRoundUpBoxVar"],
                                             self.Variables["DexterityMaxEntryVar"], self.Variables["DexterityMinEntryVar"])
        ConstitutionMod = self.GetSingleStatMod(Inst["AbilitiesAndSavingThrows"].ConstitutionEntry.AbilityEntryModifierVar, self.Variables["ConstitutionMultiplierEntryVar"], self.Variables["ConstitutionMultiplierRoundUpBoxVar"],
                                                self.Variables["ConstitutionMaxEntryVar"], self.Variables["ConstitutionMinEntryVar"])
        IntelligenceMod = self.GetSingleStatMod(Inst["AbilitiesAndSavingThrows"].IntelligenceEntry.AbilityEntryModifierVar, self.Variables["IntelligenceMultiplierEntryVar"], self.Variables["IntelligenceMultiplierRoundUpBoxVar"],
                                                self.Variables["IntelligenceMaxEntryVar"], self.Variables["IntelligenceMinEntryVar"])
        WisdomMod = self.GetSingleStatMod(Inst["AbilitiesAndSavingThrows"].WisdomEntry.AbilityEntryModifierVar, self.Variables["WisdomMultiplierEntryVar"], self.Variables["WisdomMultiplierRoundUpBoxVar"],
                                          self.Variables["WisdomMaxEntryVar"], self.Variables["WisdomMinEntryVar"])
        CharismaMod = self.GetSingleStatMod(Inst["AbilitiesAndSavingThrows"].CharismaEntry.AbilityEntryModifierVar, self.Variables["CharismaMultiplierEntryVar"], self.Variables["CharismaMultiplierRoundUpBoxVar"],
                                            self.Variables["CharismaMaxEntryVar"], self.Variables["CharismaMinEntryVar"])
        ProficiencyMod = self.GetSingleStatMod(CharacterSheetInst.ProficiencyBonusEntryVar, self.Variables["ProficiencyMultiplierEntryVar"], self.Variables["ProficiencyMultiplierRoundUpBoxVar"],
...
class CreatureData:
    def __init__(self, master, DialogMode=False, DialogData=None):
        ...
        self.SizeTypeTagsAndAlignmentFrame = Frame(self.WidgetMaster)
        ...
        # Abilities
        self.AbilitiesFrame = LabelFrame(self.WidgetMaster, text="Ability Scores:")
        ...
        self.SavingThrowsField = ScrolledText(self.SavingThrowsFrame, Width=383, Height=120)
        ...
        self.VulnerabilitiesResistancesAndImmunitiesField = ScrolledText(self.VulnerabilitiesResistancesAndImmunitiesFrame, Width=383, Height=120)
...
class MenuBar:
    def __init__(self, master):
        ...
        if WindowInst.Mode in ["EncounterManager", "CharacterSheet"]:
            self.FileMenu.add_separator()
        ...
        if WindowInst.Mode is "CharacterSheet":
            ...
            self.CommandsMenu.add_command(label="Update Stats and Inventory (Ctrl+D)", command=CharacterSheetInst.UpdateStatsAndInventory)
...
class ModeSelect(Tk):
    def __init__(self):
        ...
        self.EncounterManagerModeButton.grid(row=2, column=0, sticky=NSEW, padx=2, pady=2)
        ...
        self.CoinCalculatorModeButton.grid(row=4, column=0, sticky=NSEW, padx=2, pady=2)
...
class Window(Tk):
    ...
    def UpdateWindowTitle(self):
        # Prefix
        if self.Mode is "CreatureDataUtility":
        ...
        if self.Mode in ["CharacterSheet", "EncounterManager", "CreatureDataUtility", "DiceRoller"]:
    ...
    def ConfigureBindings(self):
        ...
        # Saving and Opening
        if self.Mode in ["CreatureDataUtility", "DiceRoller", "EncounterManager", "CharacterSheet"]:
            # Save and Open Keystrokes
            self.bind("<Control-s>", lambda event: SavingAndOpeningInst.SaveButton())
            self.bind("<Control-S>", lambda event: SavingAndOpeningInst.SaveButton(SaveAs=True))
            self.bind("<Control-o>", lambda event: SavingAndOpeningInst.OpenButton())
            self.bind("<Control-n>", lambda event: SavingAndOpeningInst.NewButton())

            # Track Modified Fields
            for Field in SavingAndOpeningInst.SavedData.values():
                FieldType = type(Field)
                if FieldType == StringVar or FieldType == BooleanVar:
                    Field.trace_add("write", lambda a, b, c: SavingAndOpeningInst.SavedDataModified())
                elif FieldType == ScrolledText:
                    Field.Text.bind("<<TextModified>>", lambda event: SavingAndOpeningInst.SavedDataModified())

        # Dice Roller
        if self.Mode in ["DiceRoller", "EncounterManager", "CharacterSheet"]:
            # Scrolling
            DiceRollerInst.PresetRollsInst.PresetRollsScrolledCanvas.BindEnterAndLeaveToBindMouseWheel()

            # Tooltips
            StatusBarInst.TooltipConfig(DiceRollerInst.DiceNumberEntry, "Scroll the mouse wheel or type to change the number of dice.")
            StatusBarInst.TooltipConfig(DiceRollerInst.DieTypeEntry, "Scroll the mouse wheel or type to change the die type.")
            StatusBarInst.TooltipConfig(DiceRollerInst.ModifierEntry, "Scroll the mouse wheel or type to change the modifier.")
            StatusBarInst.TooltipConfig(DiceRollerInst.ResultsField.ScrolledTextFrame, "Left-click to copy results to the clipboard.  Right-click to clear.")
            StatusBarInst.TooltipConfig(Inst["PresetRolls"].PresetRollsScrolledCanvasNameHeader, SortTooltipString)
            StatusBarInst.TooltipConfig(Inst["PresetRolls"].PresetRollsScrolledCanvasSortOrderHeader, SortTooltipString)

        # Encounter Manager
        if self.Mode is "EncounterManager":
            # Scrolling
            ...
            WindowInst.bind("<Control-r>", lambda event: InitiativeOrderInst.NewRound())
            WindowInst.bind("<Control-t>", lambda event: InitiativeOrderInst.NextTurn())
            WindowInst.bind("<Control-T>", lambda event: InitiativeOrderInst.ClearTurns())
            WindowInst.bind("<Control-i>", lambda event: InitiativeOrderInst.SortInitiativeOrder())
            StatusBarInst.TooltipConfig(InitiativeOrderInst.NewRoundButton, "Keyboard Shortcut:  Ctrl+R")
            StatusBarInst.TooltipConfig(InitiativeOrderInst.NextTurnButton, "Keyboard Shortcut:  Ctrl+T")
            StatusBarInst.TooltipConfig(InitiativeOrderInst.ClearTurnsButton, "Keyboard Shortcut:  Ctrl+Shift+T")
            StatusBarInst.TooltipConfig(InitiativeOrderInst.SortInitiativeOrderButton, "Keyboard Shortcut:  Ctrl+I")

            # Initiative Entry Tooltips
            for Entry in InitiativeOrderInst.InitiativeEntriesList:
                StatusBarInst.TooltipConfig(Entry.InitiativeEntryResultEntry, "Right-click to toggle turn taken.")
                StatusBarInst.TooltipConfig(Entry.InitiativeEntryNameEntry, "Right-click to set additional creature info.  Shift+right-click to duplicate.  Ctrl+right-click to clear.")
                StatusBarInst.TooltipConfig(Entry.InitiativeEntryCurrentHPEntry, "Right-click to damage.  Shift+right-click to heal.  Control+right-click to toggle dead.")

        # Character Sheet
        if self.Mode is "CharacterSheet":
            # Update Stats Keystroke
            self.bind("<Control-d>", lambda event: CharacterSheetInst.UpdateStatsAndInventory())

            # Scrolling
            Inst["FeaturesAndCreatureStats"].FeaturesScrolledCanvas.BindEnterAndLeaveToBindMouseWheel()
            for List in Inst["Spellcasting"].SpellListsList:
                List.SpellListScrolledCanvas.BindEnterAndLeaveToBindMouseWheel()
            Inst["Inventory"].InventoryListScrolledCanvas.BindEnterAndLeaveToBindMouseWheel()

            # AC Tooltip
            StatusBarInst.TooltipConfig(Inst["CombatAndFeatures"].ACEntry, "Left-click on AC to set data.")

            # Features Tooltips
            StatusBarInst.TooltipConfig(Inst["FeaturesAndCreatureStats"].NameHeader, SortTooltipString)
            StatusBarInst.TooltipConfig(Inst["FeaturesAndCreatureStats"].SortOrderHeader, SortTooltipString)
            for Entry in Inst["FeaturesAndCreatureStats"].FeatureOrCreatureStatsEntriesList:
                StatusBarInst.TooltipConfig(Entry.NameEntry, "Left-click on a feature or creature stats entry to set a feature.  Right-click to set creature stats.")

            # Spellcasting Tooltips
            StatusBarInst.TooltipConfig(Inst["Spellcasting"].SpellPointsRemainingEntry, "Left-click on the spell points remaining to expend points.  Right-click to restore.")
            for SpellList in Inst["Spellcasting"].SpellListsList:
                StatusBarInst.TooltipConfig(SpellList.PreparedHeader, SortTooltipString[:-29])
                StatusBarInst.TooltipConfig(SpellList.NameHeader, SortTooltipString)
                StatusBarInst.TooltipConfig(SpellList.SortOrderHeader, SortTooltipString)
                for Entry in SpellList.SpellListEntriesList:
                    StatusBarInst.TooltipConfig(Entry.NameEntry, "Left-click on a spell list entry to set a name and description.")

            # Inventory Tooltips
            ConsumptionRateTooltip = "Left-click to set the consumption rate per day for supplies."
            StatusBarInst.TooltipConfig(Inst["Inventory"].InventoryListNameHeader, SortTooltipString)
            StatusBarInst.TooltipConfig(Inst["Inventory"].InventoryListCountHeader, SortTooltipString)
            StatusBarInst.TooltipConfig(Inst["Inventory"].InventoryListUnitWeightHeader, SortTooltipString)
            StatusBarInst.TooltipConfig(Inst["Inventory"].InventoryListUnitValueHeader, SortTooltipString)
            StatusBarInst.TooltipConfig(Inst["Inventory"].InventoryListUnitValueDenominationHeader, SortTooltipString)
            StatusBarInst.TooltipConfig(Inst["Inventory"].InventoryListTotalWeightHeader, SortTooltipString)
            StatusBarInst.TooltipConfig(Inst["Inventory"].InventoryListTotalValueHeader, SortTooltipString)
            StatusBarInst.TooltipConfig(Inst["Inventory"].InventoryListTagHeader, SortTooltipString)
            StatusBarInst.TooltipConfig(Inst["Inventory"].InventoryListSortOrderHeader, SortTooltipString)
            StatusBarInst.TooltipConfig(Inst["Inventory"].FoodDisplay.DaysEntry, ConsumptionRateTooltip)
            StatusBarInst.TooltipConfig(Inst["Inventory"].WaterDisplay.DaysEntry, ConsumptionRateTooltip)
            for Entry in Inst["Inventory"].InventoryEntriesList:
                StatusBarInst.TooltipConfig(Entry.NameEntry, "Right-click on the name field to set magic item information.")

        # Bind Closing Protocol
        self.wm_protocol("WM_DELETE_WINDOW", lambda: self.CloseWindow(CheckForSave=self.Mode in ["CreatureDataUtility", "DiceRoller", "EncounterManager", "CharacterSheet"]))
...
if __name__ == "__main__":
    ...
    # Window Titles
    WindowTitles = {}
    WindowTitles["CharacterSheet"] = "Character Sheet"
    WindowTitles["DiceRoller"] = "Dice Roller"
    WindowTitles["EncounterManager"] = "Encounter Manager"
    WindowTitles["CreatureDataUtility"] = "Creature Data Utility"
    WindowTitles["CoinCalculator"] = "Coin Calculator"
    ...
    if WindowInst.Mode is "CoinCalculator":
        CoinCalculatorInst = CoinCalculator(WindowInst)
    elif WindowInst.Mode is "CreatureDataUtility":
        StatusBarInst = StatusBar(WindowInst, Row=1, Column=0)
        SavingAndOpeningInst = SavingAndOpening()
        MenuBarInst = MenuBar(WindowInst)
        CreatureDataInst = CreatureData(WindowInst)
    elif WindowInst.Mode is "DiceRoller":
        StatusBarInst = StatusBar(WindowInst, Row=2, Column=0)
        SavingAndOpeningInst = SavingAndOpening()
        DiceRollerInst = DiceRoller(WindowInst, Row=1, Column=0)
        MenuBarInst = MenuBar(WindowInst)
    elif WindowInst.Mode is "EncounterManager":
        StatusBarInst = StatusBar(WindowInst, Row=3, Column=0)
        SavingAndOpeningInst = SavingAndOpening()
        EncounterHeaderInst = EncounterHeader(WindowInst)
        InitiativeOrderInst = InitiativeOrder(WindowInst)
        DiceRollerInst = DiceRoller(WindowInst, Row=0, Column=1)
        MenuBarInst = MenuBar(WindowInst)
    elif WindowInst.Mode is "CharacterSheet":
        StatusBarInst = StatusBar(WindowInst, Row=3, Column=0)
        SavingAndOpeningInst = SavingAndOpening()
        CharacterSheetInst = CharacterSheet(WindowInst, Row=1, Column=0)
        DiceRollerInst = DiceRoller(WindowInst, Row=1, Column=1)
        MenuBarInst = MenuBar(WindowInst)

    # Configure Window
    WindowInst.ConfigureBindings()
