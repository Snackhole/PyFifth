...
class CharacterStatsInventoryAndNotes:
        ...
        class AbilitiesAndSavingThrowsTable:
            ...
            class AbilitiesAndSavingThrowsEntry:
                def __init__(self, master, AbilityName, List, Row):
                    ...
                    self.AbilityScoreManualBonusEntryVar = StringVar()
                    self.AbilitySavingThrowManualBonusEntryVar = StringVar()
                    ...
                    self.AbilityEntryModifier.bind("<Button-3>", self.ConfigureAbilityScoreManualBonus)
                    ...
                    self.AbilitySavingThrowModifier.bind("<Button-3>", self.ConfigureSavingThrowManualBonus)
                    ...
                    GlobalInst.SavedData[self.AbilityName + "AbilityScoreManualBonusEntryVar"] = self.AbilityScoreManualBonusEntryVar
                    GlobalInst.SavedData[self.AbilityName + "AbilitySavingThrowManualBonusEntryVar"] = self.AbilitySavingThrowManualBonusEntryVar

                def CalculateModifiers(self, ProficiencyBonus):
                    BaseAbilityModifier = int(math.floor((int(self.AbilityEntryTotalVar.get()) - 10) / 2))
                    AbilityModifier = BaseAbilityModifier + GlobalInst.GetStringVarAsNumber(self.AbilityScoreManualBonusEntryVar)
                    AbilityModifierSign = ""
                    if AbilityModifier >= 1:
                        AbilityModifierSign = "+"
                    AbilityModifierString = AbilityModifierSign + str(AbilityModifier)
                    SavingThrowModifier = BaseAbilityModifier + GlobalInst.GetStringVarAsNumber(self.AbilitySavingThrowManualBonusEntryVar)
                ...
                def ConfigureAbilityScoreManualBonus(self, event):
                    # Test Level Input Validity
                    if CharacterSheetHeaderInst.ValidLevelEntry():
                        pass
                    else:
                    ...
                    # Create Config Window and Wait
                    AbilityScoreManualBonusInst = ManualBonusData(root, self.AbilityScoreManualBonusEntryVar, self.AbilityName + " Ability Modifier")
                    root.wait_window(AbilityScoreManualBonusInst.Window)

                    # Handle Values
                    if AbilityScoreManualBonusInst.DataSubmitted.get():
                        AbilityScoreManualBonusInst.GetData(self.AbilityScoreManualBonusEntryVar)

                    # Update Stats and Inventory
                    GlobalInst.UpdateStatsAndInventory()

                def ConfigureSavingThrowManualBonus(self, event):
                    # Test Level Input Validity
                    if CharacterSheetHeaderInst.ValidLevelEntry():
                        pass
                    else:
                    ...
                    # Create Config Window and Wait
                    SavingThrowManualBonusInst = ManualBonusData(root, self.AbilitySavingThrowManualBonusEntryVar, self.AbilityName + " Saving Throw")
                    root.wait_window(SavingThrowManualBonusInst.Window)

                    # Handle Values
                    if SavingThrowManualBonusInst.DataSubmitted.get():
                        SavingThrowManualBonusInst.GetData(self.AbilitySavingThrowManualBonusEntryVar)
        ...
        class SkillsTable:
            ...
            class SkillsEntry:
                def __init__(self, master, SkillName, List, Row):
                    ...
                    self.SkillManualBonusEntryVar = StringVar()
                    ...
                    self.ModifierEntry.bind("<Button-3>", self.ConfigureSkillManualBonus)

                    # Add Saved Fields to Saved Data Dictionary
                    GlobalInst.SavedData[self.SkillName + "SkillProficiency1"] = self.ProficiencyBox1Var
                    GlobalInst.SavedData[self.SkillName + "SkillProficiency2"] = self.ProficiencyBox2Var
                    GlobalInst.SavedData[self.SkillName + "SkillManualBonusEntryVar"] = self.SkillManualBonusEntryVar
                ...
                def CalculateSkillModifier(self, Ability, ProficiencyBonus):
                    ...
                    if self.ProficiencyBox2Var.get():
                        Modifier += ProficiencyBonus
                    Modifier += int(Ability) + GlobalInst.GetStringVarAsNumber(self.SkillManualBonusEntryVar)
                    ModifierSign = ""
                    if Modifier >= 1:
                        ModifierSign = "+"
                    self.TotalModifierVar.set(ModifierSign + str(Modifier))

                def ConfigureSkillManualBonus(self, event):
                    # Test Level Input Validity
                    if CharacterSheetHeaderInst.ValidLevelEntry():
                        pass
                    else:
                    ...
                    # Create Config Window and Wait
                    SavingThrowManualBonusInst = ManualBonusData(root, self.SkillManualBonusEntryVar, self.SkillName + " Data")
                    root.wait_window(SavingThrowManualBonusInst.Window)

                    # Handle Values
                    if SavingThrowManualBonusInst.DataSubmitted.get():
                        SavingThrowManualBonusInst.GetData(self.SkillManualBonusEntryVar)
    ...
    class CombatAndFeatures:
        def __init__(self, master):
            ...
            # Notes Dimensions
            self.NotesHeight = 100
            self.NotesWidth = 344

            # Offense Notes
            self.OffenseNotesFrame = LabelFrame(master, text="Offense Notes:")
            self.OffenseNotesFrame.grid(row=5, column=1, columnspan=3)
            self.OffenseNotes = ScrolledText(self.OffenseNotesFrame, Height=self.NotesHeight, Width=self.NotesWidth)
            self.OffenseNotes.grid(row=0, column=0)

            # Defense Notes
            self.DefenseNotesFrame = LabelFrame(master, text="Defense Notes:")
            self.DefenseNotesFrame.grid(row=7, column=1, columnspan=3)
            self.DefenseNotes = ScrolledText(self.DefenseNotesFrame, Height=self.NotesHeight, Width=self.NotesWidth)
            ...
            self.FeaturesNotesFrame.grid(row=7, column=5)
            self.FeaturesNotes = ScrolledText(self.FeaturesNotesFrame, Height=self.NotesHeight, Width=self.NotesWidth)
        ...
        class FeaturesAndCreatureStats:
            ...
            class FeatureOrCreatureStatsEntry:
                ...
                class CreatureStatsConfig:
                    ...
                    def Import(self):
                        ToolbarAndStatusBarInst.StatusBarSetText("Importing creature file...", Lock=True)
                        ...
                        if OpenFileName != "":
                            ...
                            if self.OpenErrors:
                                ...
                                self.OpenErrors = False
                                self.OpenErrorsString = ""
                            ToolbarAndStatusBarInst.StatusBarSetText("Imported file:  " + os.path.basename(OpenFileName), Lock=True)
                            root.after(2000, lambda: ToolbarAndStatusBarInst.StatusBarSetText("Status", Unlock=True))
                        else:
                            ToolbarAndStatusBarInst.StatusBarSetText("No file imported!", Lock=True)
                            root.after(2000, lambda: ToolbarAndStatusBarInst.StatusBarSetText("Status", Unlock=True))
                    ...
                    def Export(self):
                        ToolbarAndStatusBarInst.StatusBarSetText("Exporting creature file...", Lock=True)
                        ...
                        if SaveFileName != "":
                            ...
                            GlobalInst.DeleteFile(TextFileName)
                            sleep(0.5)
                            ToolbarAndStatusBarInst.StatusBarSetText("File saved as:  " + os.path.basename(SaveFileName), Lock=True)
                            root.after(2000, lambda: ToolbarAndStatusBarInst.StatusBarSetText("Status", Unlock=True))
                        else:
                            ToolbarAndStatusBarInst.StatusBarSetText("No file saved!", Lock=True)
                            root.after(2000, lambda: ToolbarAndStatusBarInst.StatusBarSetText("Status", Unlock=True))
    ...
    class Inventory:
        ...
        class SupplyDisplay:
            def __init__(self, master, ConsumptionRateDefault, Tag):
                ...
                GlobalInst.SavedData[self.Tag + "ConsumptionRateVar"] = self.ConsumptionRateVar
            ...
            def SetConsumptionRate(self, event):
                ...
                ConsumptionRatePromptInst = self.ConsumptionRatePrompt(root, self.ConsumptionRateVar, self.Tag)
                root.wait_window(ConsumptionRatePromptInst.Window)
...
class DiceRoller:
    def __init__(self, master):
        ...
        # Preset Rolls
        self.PresetRollsInst = self.PresetRolls(self.DiceRollerFrame, self.ResultsField)
    ...
    class PresetRolls:
        def __init__(self, master, ResultsField):
            # Store Parameters
            ...
            self.DiceRollerFields["CritRangeEntryVar"] = GlobalInst.CritRangeEntryVar
        ...
        class PresetRollEntry:
            def __init__(self, master, List, ScrollingDisabledVar, SortOrderValuesTuple, DiceRollerFields, Row):
                ...
                self.PresetRollConfigSubmitted = BooleanVar()
                self.PresetRollStrengthBoxVar = BooleanVar()
                self.PresetRollDexterityBoxVar = BooleanVar()
                self.PresetRollConstitutionBoxVar = BooleanVar()
                self.PresetRollIntelligenceBoxVar = BooleanVar()
                self.PresetRollWisdomBoxVar = BooleanVar()
                self.PresetRollCharismaBoxVar = BooleanVar()
                self.PresetRollProficiencyBoxVar = BooleanVar()
                self.PresetRollManualBonusEntryVar = StringVar()
            ...
            def ConfigureModifier(self):
                ...
                # Create Config Window and Wait
                PresetRollModifierConfigInst = self.PresetRollModifierConfig(root, self.PresetRollStrengthBoxVar, self.PresetRollDexterityBoxVar, self.PresetRollConstitutionBoxVar, self.PresetRollIntelligenceBoxVar,
                                                                             self.PresetRollWisdomBoxVar, self.PresetRollCharismaBoxVar,
                                                                             self.PresetRollProficiencyBoxVar, self.PresetRollManualBonusEntryVar)
                root.wait_window(PresetRollModifierConfigInst.Window)

                # Handle Entry
                if PresetRollModifierConfigInst.NewConfigSubmitted.get():
                    # Handle Variables
                    self.PresetRollConfigSubmitted.set(True)
                    self.PresetRollStrengthBoxVar.set(PresetRollModifierConfigInst.StrengthBoxVar.get())
                    self.PresetRollDexterityBoxVar.set(PresetRollModifierConfigInst.DexterityBoxVar.get())
                    self.PresetRollConstitutionBoxVar.set(PresetRollModifierConfigInst.ConstitutionBoxVar.get())
                    self.PresetRollIntelligenceBoxVar.set(PresetRollModifierConfigInst.IntelligenceBoxVar.get())
                    self.PresetRollWisdomBoxVar.set(PresetRollModifierConfigInst.WisdomBoxVar.get())
                    self.PresetRollCharismaBoxVar.set(PresetRollModifierConfigInst.CharismaBoxVar.get())
                    self.PresetRollProficiencyBoxVar.set(PresetRollModifierConfigInst.ProficiencyBoxVar.get())
                    self.PresetRollManualBonusEntryVar.set(PresetRollModifierConfigInst.ManualBonusEntryVar.get())

                    # Update Stats and Inventory
                    GlobalInst.UpdateStatsAndInventory()
                elif PresetRollModifierConfigInst.ConfigCleared.get():
                    # Handle Variables
                    self.PresetRollConfigSubmitted.set(False)
                    self.PresetRollStrengthBoxVar.set(False)
                    self.PresetRollDexterityBoxVar.set(False)
                    self.PresetRollConstitutionBoxVar.set(False)
                    self.PresetRollIntelligenceBoxVar.set(False)
                    self.PresetRollWisdomBoxVar.set(False)
                    self.PresetRollCharismaBoxVar.set(False)
                    self.PresetRollProficiencyBoxVar.set(False)
                    self.PresetRollManualBonusEntryVar.set("")
                    self.ConfigureModifierEntry(False)

            def SetConfiguredModifier(self):
                ConfigSubmitted = self.PresetRollConfigSubmitted.get()
                if ConfigSubmitted:
                    Modifier = 0
                    if self.PresetRollStrengthBoxVar.get():
                    ...
                    if self.PresetRollDexterityBoxVar.get():
                    ...
                    if self.PresetRollConstitutionBoxVar.get():
                    ...
                    if self.PresetRollIntelligenceBoxVar.get():
                    ...
                    if self.PresetRollWisdomBoxVar.get():
                    ...
                    if self.PresetRollCharismaBoxVar.get():
                    ...
                    if self.PresetRollProficiencyBoxVar.get():
                        Modifier += GlobalInst.GetStringVarAsNumber(CharacterSheetHeaderInst.ProficiencyBonusEntryVar)
                    Modifier += GlobalInst.GetStringVarAsNumber(self.PresetRollManualBonusEntryVar)
                    self.PresetRollModifierEntryVar.set(str(Modifier))
                self.ConfigureModifierEntry(ConfigSubmitted)

            def ConfigureModifierEntry(self, Submitted):
                if Submitted:
            ...
            def Display(self, Row):
                ...
                GlobalInst.SavedData["PresetRollConfigSubmitted" + str(self.Row)] = self.PresetRollConfigSubmitted
                GlobalInst.SavedData["PresetRollStrengthBoxVar" + str(self.Row)] = self.PresetRollStrengthBoxVar
                GlobalInst.SavedData["PresetRollDexterityBoxVar" + str(self.Row)] = self.PresetRollDexterityBoxVar
                GlobalInst.SavedData["PresetRollConstitutionBoxVar" + str(self.Row)] = self.PresetRollConstitutionBoxVar
                GlobalInst.SavedData["PresetRollIntelligenceBoxVar" + str(self.Row)] = self.PresetRollIntelligenceBoxVar
                GlobalInst.SavedData["PresetRollWisdomBoxVar" + str(self.Row)] = self.PresetRollWisdomBoxVar
                GlobalInst.SavedData["PresetRollCharismaBoxVar" + str(self.Row)] = self.PresetRollCharismaBoxVar
                GlobalInst.SavedData["PresetRollProficiencyBoxVar" + str(self.Row)] = self.PresetRollProficiencyBoxVar
                GlobalInst.SavedData["PresetRollManualBonusEntryVar" + str(self.Row)] = self.PresetRollManualBonusEntryVar
...
class MenuAndStatusBar:
    def __init__(self, master):
        ...
        self.FileMenu.add_separator()
        self.FileMenu.add_command(label="Import Dice Roller Data", command=self.ImportDiceRoller)
        self.FileMenu.add_command(label="Export Dice Roller Data", command=self.ExportDiceRoller)
        self.FileMenu.add_separator()
        ...
        self.CommandsMenu.add_command(label="Update Stats and Inventory (Ctrl+D)", command=GlobalInst.UpdateStatsAndInventory)
        self.CommandsMenu.add_separator()
        self.CommandsMenu.add_command(label="Clear Inventory", command=CharacterStatsInventoryAndNotesInst.InventoryInst.Clear)
        self.MenuBar.add_cascade(label="Commands", menu=self.CommandsMenu)

        # Coin Calculator
        self.MenuBar.add_command(label="Coin Calculator", command=CharacterStatsInventoryAndNotesInst.InventoryInst.OpenCoinCalculator)

        # Settings Menu
        self.MenuBar.add_command(label="Settings", command=GlobalInst.Settings)
    ...
    def SaveButton(self, SaveAs=False):
        ...
        if SaveFileName != "":
            ...
            GlobalInst.DeleteFile(TextFileName)
            if PortraitSelected:
                GlobalInst.DeleteFile(PortraitFileName)
    ...
    def ExportDiceRoller(self):
        ...
        if not ExportConfirm:
            return
        self.StatusBarSetText("Exporting rolls file...", Lock=True)
    ...
    def ExportDiceRollerData(self, File):
        for Tag, Field in DiceRollerInst.PresetRollsInst.DiceRollerFields.items():
            File.write(json.dumps({Tag: Field.get()}) + "\n")
    ...
    def OpenData(self, File):
        ...
        for Entry in DiceRollerInst.PresetRollsInst.PresetRollsList:
            Entry.ConfigureModifierEntry(Entry.PresetRollConfigSubmitted.get())
    ...
    def NewButton(self):
        ...
        for Entry in DiceRollerInst.PresetRollsInst.PresetRollsList:
            Entry.ConfigureModifierEntry(False)
        ...
        CharacterStatsInventoryAndNotesInst.InventoryInst.FoodDisplay.LoadEntryVar.set("")
        CharacterStatsInventoryAndNotesInst.InventoryInst.FoodDisplay.DaysEntryVar.set("")
        CharacterStatsInventoryAndNotesInst.InventoryInst.FoodDisplay.ConsumptionRateVar.set("1")
        CharacterStatsInventoryAndNotesInst.InventoryInst.WaterDisplay.LoadEntryVar.set("")
        CharacterStatsInventoryAndNotesInst.InventoryInst.WaterDisplay.DaysEntryVar.set("")
        CharacterStatsInventoryAndNotesInst.InventoryInst.WaterDisplay.ConsumptionRateVar.set("8")
        ...
        AllAbilityScoreDerivativesList = CharacterStatsInventoryAndNotesInst.CombatAndFeaturesInst.AbilityScoreDerivativesList + CharacterStatsInventoryAndNotesInst.SpellcastingInst.SpellcastingAbilitiesList
    ...
    def ImportDiceRoller(self):
        ...
        if not ImportConfirm:
            return
        self.StatusBarSetText("Importing roll file...", Lock=True)
        ...
        if OpenFileName != "":
            ...
            if self.OpenErrors:
                ...
                self.OpenErrors = False
                self.OpenErrorsString = ""
            self.StatusBarSetText("Imported file:  " + os.path.basename(OpenFileName), Lock=True)
            root.after(2000, lambda: self.StatusBarSetText("Status", Unlock=True))
        else:
            self.StatusBarSetText("No file imported!", Lock=True)
            root.after(2000, lambda: self.StatusBarSetText("Status", Unlock=True))
    ...
    def ImportDiceRollerData(self, File):
        for Line in File:
            if Line != "":
                ...
                for Tag, Field in LoadedLine.items():
                    try:
                        DiceRollerInst.PresetRollsInst.DiceRollerFields[Tag].set(Field)
                    except KeyError:
                        self.OpenErrors = True
                        self.OpenErrorsString += Line
        for Entry in DiceRollerInst.PresetRollsInst.PresetRollsList:
            Entry.ConfigSubmitted.set(False)
            Entry.StrengthBoxVar.set(False)
            Entry.DexterityBoxVar.set(False)
            Entry.ConstitutionBoxVar.set(False)
            Entry.IntelligenceBoxVar.set(False)
            Entry.WisdomBoxVar.set(False)
            Entry.CharismaBoxVar.set(False)
            Entry.ProficiencyBoxVar.set(False)
            Entry.ManualBonusEntryVar.set("")
            Entry.ConfigureModifierEntry(False)
...
class Global:
    ...
    def UpdateStatsAndInventory(self):
        ...
        for Ability in AllAbilityScoreDerivativesList:
            ...
            AbilityModifier = 0
            AttackModifierManualBonus = GlobalInst.GetStringVarAsNumber(Ability.AttackModifierManualBonusVar)
            SaveDCManualBonus = GlobalInst.GetStringVarAsNumber(Ability.SaveDCManualBonusVar)
            if AbilityScore != "":
                if AbilityScore == "STR":
                    AbilityModifier = StrengthModifier
                    ...
                    AbilityModifier = CharismaModifier
                AttackModifier = AbilityModifier + ProficiencyModifier + AttackModifierManualBonus
                AttackModifierSign = ""
                if AttackModifier > 0:
                    AttackModifierSign = "+"
                Ability.AttackModifierEntryVar.set(AttackModifierSign + str(AttackModifier))
                SaveDC = AbilityModifier + ProficiencyModifier + SaveDCManualBonus + 8
    ...
    def ConfigureBindings(self):
        ...
        AbilityAndSavingThrowTooltipString = "Left-click on an ability or saving throw modifier to roll 1d20 with it.  Right-click to set a bonus."
        for Entry in CharacterStatsInventoryAndNotesInst.AbilitiesAndSkillsInst.AbilitiesAndSavingThrowsInst.AbilityEntriesList:
            ToolbarAndStatusBarInst.TooltipConfig(Entry.AbilityEntryModifier, AbilityAndSavingThrowTooltipString)
            ToolbarAndStatusBarInst.TooltipConfig(Entry.AbilitySavingThrowModifier, AbilityAndSavingThrowTooltipString)

        # Skill Roll Tooltips
        for Entry in CharacterStatsInventoryAndNotesInst.AbilitiesAndSkillsInst.SkillsInst.SkillsEntriesList:
            ToolbarAndStatusBarInst.TooltipConfig(Entry.ModifierEntry, "Left-click on a skill modifier to roll 1d20 with it.  Right-click to set a bonus.")
        ...
        # Ability Score Derivatives Tooltips
        for Ability in CharacterStatsInventoryAndNotesInst.CombatAndFeaturesInst.AbilityScoreDerivativesList:
            ToolbarAndStatusBarInst.TooltipConfig(Ability.SaveDCEntry, "Left-click on a save DC to set a bonus.")
            ToolbarAndStatusBarInst.TooltipConfig(Ability.AttackModifierEntry, "Left-click on an attack modifier to roll 1d20 with it.  Right-click to set a bonus.")

        # Spellcasting Tooltips
        for Ability in CharacterStatsInventoryAndNotesInst.SpellcastingInst.SpellcastingAbilitiesList:
            ToolbarAndStatusBarInst.TooltipConfig(Ability.SaveDCEntry, "Left-click on a spell save DC to set a bonus.")
            ToolbarAndStatusBarInst.TooltipConfig(Ability.AttackModifierEntry, "Left-click on a spell attack modifier to roll 1d20 with it.  Right-click to set a bonus.")
        ...
        ToolbarAndStatusBarInst.TooltipConfig(CharacterStatsInventoryAndNotesInst.SpellcastingInst.SpellPointsRemainingEntry, "Left-click on the spell points remaining to expend points.  Right-click to restore.")
        ...
        # Inventory Tooltips
        ConsumptionRateTooltip = "Left-click to set the consumption rate per day for supplies."
        ...
        ToolbarAndStatusBarInst.TooltipConfig(CharacterStatsInventoryAndNotesInst.InventoryInst.FoodDisplay.DaysEntry, ConsumptionRateTooltip)
        ToolbarAndStatusBarInst.TooltipConfig(CharacterStatsInventoryAndNotesInst.InventoryInst.WaterDisplay.DaysEntry, ConsumptionRateTooltip)
        for Entry in CharacterStatsInventoryAndNotesInst.InventoryInst.InventoryEntriesList:
...
class AbilityScoreDerivatives:
    def __init__(self, master, List, SaveTagPrefix, Column, AttackTypeStringSuffix=""):
        ...
        self.AttackModifierManualBonusVar = StringVar()
        self.SaveDCManualBonusVar = StringVar()
        ...
        # Save DC
        self.SaveDCEntry = Entry(master, justify=CENTER, state=DISABLED, disabledbackground=GlobalInst.ButtonColor, disabledforeground="black", width=2, textvariable=self.SaveDCEntryVar, cursor="arrow")
        self.SaveDCEntry.grid(row=1, column=self.Column, padx=2, pady=2, sticky=NSEW)
        self.SaveDCEntry.bind("<Button-1>", self.ConfigureSaveDC)

        # Attack Modifier
        self.AttackModifierEntry = Entry(master, justify=CENTER, state=DISABLED, disabledbackground=GlobalInst.ButtonColor, disabledforeground="black", width=2, textvariable=self.AttackModifierEntryVar, cursor="dotbox")
        ...
        self.AttackModifierEntry.bind("<Button-3>", self.ConfigureAttackModifier)

        # Add Saved Fields to Saved Data Dictionary
        GlobalInst.SavedData[self.SaveTagPrefix + "AbilitySelectionDropdownVar" + str(self.Column)] = self.AbilityScoreSelectionDropdownVar
        GlobalInst.SavedData[self.SaveTagPrefix + "AttackModifierManualBonusVar" + str(self.Column)] = self.AttackModifierManualBonusVar
        GlobalInst.SavedData[self.SaveTagPrefix + "SaveDCManualBonusVar" + str(self.Column)] = self.SaveDCManualBonusVar
    ...
    def ConfigureSaveDC(self, event):
        # Test Level Input Validity
        if CharacterSheetHeaderInst.ValidLevelEntry():
            pass
        else:
        ...
        # Create Config Window and Wait
        SaveDCDataInst = ManualBonusData(root, self.SaveDCManualBonusVar, "Save DC")
        root.wait_window(SaveDCDataInst.Window)

        # Handle Values
        if SaveDCDataInst.DataSubmitted.get():
            SaveDCDataInst.GetData(self.SaveDCManualBonusVar)

        # Update Stats and Inventory
        GlobalInst.UpdateStatsAndInventory()

    def ConfigureAttackModifier(self, event):
        # Test Level Input Validity
        if CharacterSheetHeaderInst.ValidLevelEntry():
            pass
        else:
        ...
        # Create Config Window and Wait
        AttackModifierDataInst = ManualBonusData(root, self.AttackModifierManualBonusVar, "Attack Modifier")
        root.wait_window(AttackModifierDataInst.Window)

        # Handle Values
        if AttackModifierDataInst.DataSubmitted.get():
            AttackModifierDataInst.GetData(self.AttackModifierManualBonusVar)
