...
class HoardEditItemDialog(QDialog):
    def __init__(self, HoardWindow, Inventory, ItemIndex, AddMode=False):
        super().__init__(parent=HoardWindow)
        ...
        self.setWindowTitle(self.HoardWindow.ScriptName)
        self.setWindowIcon(self.HoardWindow.WindowIcon)
    ...
    def ValidInput(self, Alert=False):
        if self.ItemNameLineEdit.text() == "":
            if Alert:
                self.HoardWindow.DisplayMessageBox("Items must have a name.", Icon=QMessageBox.Icon.Warning, Parent=self)
            return False
        return True
